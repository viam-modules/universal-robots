...
| `reject_move_request_threshold_deg` | float | Optional, but required for streaming moves | Rejects move requests when the difference between the current position and first waypoint is above threshold |
...
| `segmentation_threshold` | float | Optional | Threshold for detecting direction reversals (cusps) in waypoint paths. The trajectory planner segments the path at points where the cosine similarity between consecutive direction vectors is close to -1 (i.e. `\|dot(ab, bc) + 1\| < threshold`). Lower values detect only sharp reversals; higher values are more sensitive. **Default 0.005** (range: > 0, <= 0.01) |
| `enable_new_trajectory_planner` | bool | Optional | Enables the trajex/TOTG trajectory generator. **Default true** |
