...
void URArm::move_tool_space_(std::shared_lock<std::shared_mutex> config_rlock, pose p, const std::string& unix_time) {
    ...
    // create a trajectory sample point dictating where we want to move to
    trajectory_sample_point sample{
        target_pose,         // position
        {0, 0, 0, 0, 0, 0},  // velocities
        0.0,                 // timestep
    };

    // take the sample point and hand that over to the ur arm
    // TODO(RSDK-12185): Capture pose we ask the arm to visit
    // https://viam.atlassian.net/browse/RSDK-12185
    const std::string& path = current_state_->csv_output_path();
    write_trajectory_to_file(move_to_position_trajectory_filename(path, unix_time), {sample});
    ...
    auto trajectory_completion_future = [&, config_rlock = std::move(our_config_rlock), ajp_of = std::move(ajp_of)]() mutable {
        return current_state_->enqueue_move_request(current_move_epoch, MoveCommand{std::move(sample)}, std::move(ajp_of));
    }();
...
void URArm::move_joint_space_(std::shared_lock<std::shared_mutex> config_rlock,
    ...
    auto trajectory_completion_future = [&, config_rlock = std::move(our_config_rlock), ajp_of = std::move(ajp_of)]() mutable {
        return current_state_->enqueue_move_request(current_move_epoch, MoveCommand{std::move(samples)}, std::move(ajp_of));
    }();
