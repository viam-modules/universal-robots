...
class uniform_sampler {
   public:
    /// Construct uniform sampler with given time step
    /// @param dt Time interval between samples
    explicit uniform_sampler(trajectory::seconds dt);

    /// Create uniform sampler with adjusted timestep to hit duration endpoint exactly
    ///
    ...
    /// Create uniform sampler with adjusted timestep to hit trajectory endpoint exactly
    ///
    ...
    /// Calculate adjusted timestep for quantized sampling
    ///
    ...
    /// Get next sample, advancing cursor by dt
